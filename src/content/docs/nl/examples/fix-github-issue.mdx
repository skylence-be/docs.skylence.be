---
title: GitHub-issue oplossen
description: Classificeer issue, implementeer oplossing in worktree, multi-agent review, zelf verbeteren, concept-PR openen.
sidebar:
  order: 1
---

Gebruik dit wanneer je wil dat Skylence een issue uit je backlog oppakt en de volledige engineeringcyclus afhandelt — het probleem begrijpen, code schrijven, tests draaien en een pull request openen — zonder dat jij iets hoeft te doen behalve een label toevoegen.

De workflow is adaptief: hij detecteert of het issue een bug of een feature is en kiest de juiste aanpak. Bugs krijgen eerst een root-cause onderzoek. Features krijgen een implementatieplan. Beide doorlopen een meervoudige code review voordat de PR wordt aangemaakt, en kritieke bevindingen worden automatisch verholpen.

**Trigger:** Voeg label `ready-for-sky` toe aan issue.

**Wat het doet:**

1. **Classificeer** (Haiku) — bepaalt type: bug / feature / enhancement.
2. **Onderzoek of plan** (Opus) — root-cause analyse voor bugs; implementatieplan voor features.
3. **Implementeer** (Opus, geïsoleerde worktree) — voert oplossing of plan uit, draait tests, commit, pusht.
4. **Test** (bash) — draait `make test` als verificatiepoort.
5. **Review** (Haiku routeert → Sonnet-agents) — code review, foutafhandeling, testdekking lopen parallel op basis van diff-inhoud.
6. **Synthetiseer** (Sonnet) — consolideert bevindingen per ernst.
7. **Zelf verbeteren** (Opus) — verhelpt alle CRITICAL en MAJOR bevindingen.
8. **PR aanmaken** (Sonnet) — opent concept-pull request.
