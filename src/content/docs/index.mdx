...
Let's be honest: nobody is hand-writing these workflow files. Everyone reaches for an LLM. If the LLM is authoring it anyway, it might as well write prompts tuned for itself instead of dressed up for us. The prose between the blocks exists purely so a human can follow along and sign off before it runs.
