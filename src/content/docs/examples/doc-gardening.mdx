---
title: Doc Gardening
description: Scan docs for stale content, flag broken references, open tracking issue with checklist.
sidebar:
  order: 6
---

Use this to audit your documentation for staleness without reading through everything manually. The workflow checks each doc against the current codebase and flags three kinds of problems: references to symbols or files that no longer exist, quality grades that don't match the actual code state, and execution plans that have been completed but not archived.

The output is a GitHub tracking issue with a checklist — one item per stale doc. If everything is clean, no issue is opened.

Run this periodically — monthly, or after a large refactor.

**Trigger:** Add label `doc-garden` to any issue (manual trigger).

**What it does:**

1. **Scan** (bash) — lists all docs with last-commit date.
2. **Inventory** (Opus) — reads docs older than 90 days, cross-checks against codebase, flags broken references/outdated grades/finished plans.
3. **Report** (Sonnet) — opens tracking issue with findings checklist, or stops if scan is clean.
